...
#define REFRESH_RATE  20 // How many times per second should we update the throttle
...
const unsigned int THROTTLE_MAX[2] = { 50, 35 };
...
void loop() {
  ...
  int engage = getGamepadNormal(LEFT);
  for (int side = LEFT; side <= RIGHT; side++) {
    if (engage >= GAS_ENGAGE) {
      Serial.print(side);
      Serial.print(" ");
